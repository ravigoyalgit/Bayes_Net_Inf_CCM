dens_biased_sim <- function(init_seed, num_samples) {
  outfile = paste("dens_sampInvPropDeg_n", num_samples, "_", init_seed, ".RData", sep = "")
  ...
  Network_stats = 'DegreeDist' #'Density'  #keep degree dist so we can do sampling on degrees.
  strong_prior = TRUE
  ...
  degTemp <- degree(G)/ 2
  sample_deg <- sample(degTemp, num_samples, prob = 1/(degTemp+1), replace = FALSE)
  target_dens = (mean(sample_deg)/2)/(population-1)
