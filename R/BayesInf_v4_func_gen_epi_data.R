#' @title generate_epidemic_data
#' @description simulates epidemic data from epidemic parameters
#' @param beta_a = probability of infection for acute phase (default: 1/5)
#' @param beta_l = probability of infection for long-term phase (default: 1/2.5)
#' @param gamma_a = length of infection for acute phase (default: 1/10)
#' @param gamma_l = length of infection for long-term phase (default: 1/100)
#' @param population = number of individuals in the network (default: 500)
#' @param Prob_Distr_Params = (default: list(list(rep(1,10))))
#' @param Prob_Distr = (default: 'DirMult')
#' @param num_init_infected = number of individuals infected at the start of the epidemic (default: 1)
#' @param genetic_bits = length of genetic sequence (default: 1024)
#' @param Network_stats = (default: NULL)
#' @param strong_prior = (default: NULL)
#' @param num_samples = number of behavior survey samples (default: 100)
#' @param covPattern = (default: NULL)
#' @return "No Epidemic: restarting..."; list(G, P, Ia, Il, R, T_dist, Prob_Distr_Params_hyperprior, Prob_Distr_Params, CCMnet_Result[[2]]) or "ERROR: FAILED", simulated data, including contact and transmission networks, transition times, and genetic distances
#' @export
...
generate_epidemic_data <- function(beta_a = 1/5,
  ...
  if (counter < max_try) {
    ...
    if (Network_stats == 'Mixing') {
      if (strong_prior == TRUE) {
        ...
        for (j in sampled_v) {
          ...
          neighbors_j = as.numeric(igraph::V(G)[neighbors_j])
          cov_j = covPattern[as.numeric(igraph::V(G)[j])] +1
    ...
    T_dist = e1071::hamming.distance(PG_Data[,-(genetic_bits+1)])
...
#' @param num_init_infected = number of individuals infected at the start of the epidemic (default: 1)
#' @return exampleepidemic, with which nodes are infected, their parent, and their transition times (acute, long-term, and treatment)
#' @export
...
#' @title Initialize_G_P_Ia_Il_R
#' @description initializes the contact and transmission networks and transition times
#' @param population = number of individuals in the network
#' @param beta_a probability of infection for acute phase
#' @param beta_l probability of infection for long-term phase
#' @param gamma_a length of infection for acute phase
#' @param gamma_l length of infection for long-term phase
#' @param G contact network (default: NULL)
#' @param num_init_infected = number of individuals infected at the start of the epidemic (default: 1)
#' @return list(G,P,Ia,Il,R), the initialized contact and transmission networks and transition times
#' @export
...
Initialize_G_P_Ia_Il_R <- function(population, beta_a, beta_l, gamma_a, gamma_l, G = NULL, num_init_infected = 1) {
  ...
  if (beta_a == 0) {
    ...
    exampleepidemic <- epinet::SEIR.simulator(M = network::as.edgelist(intergraph::asNetwork(G)),
  ...
  P = igraph::graph_from_data_frame(p_df, directed=TRUE, vertices = nodes_attr_df)
...
#' @title gen_reporting_bias
#' @description adjust reported partners by the ratio_orig_mean_deg to simulate bias
#' @param sampled_v_deg reported partners before simulating bias
#' @return bias_sampled_v_deg, the simulated, biased reported number of partners
#' @export
