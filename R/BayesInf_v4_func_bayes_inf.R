#' @title Update_G
#' @description updates the contact network as part of a Metropolis-Hastings algorithm
...
#' @param Prob_Distr_Params probability distribution or mixing matrix for network property of interest
#' @param Network_stats network property of interest ("Degree" or "Mixing")
#' @param Prob_Distr assumed distribution ("Multinomial_Poisson")
#' @param covPattern covariance pattern, use for mixing
#' @return list(CCMnet_Result[[1]], CCMnet_Result[[2]]), updated contact network
#' @export
...
#' @title Update_P
#' @description updates the transmission network as part of a Metropolis-Hastings algorithm
#' @param G contact network
#' @param Ia vector of times of infection for each individual
#' @param Il vector of times of transition to long-term infection for each individual
#' @param R vector of times of transition to recovery for each individual
...
#' @param T viral genetic network
#' @return P, updated transmission network
#' @export
...
Update_P <- function(G,Ia,Il,R,beta_a,beta_l,gamma_a,gamma_l, T) {
  ...
  for (j in infected) {
    ...
    poss_parent_j = as.numeric(igraph::V(G)[poss_parent_j])
    Ia_j = Ia[j]
    Il_j = Il[j]
  ...
  P = igraph::graph_from_data_frame(new_P, directed=TRUE, vertices = nodes_attr_df)

  return(P)
}

#' @title Update_Ia
#' @description updates acute infection times
#' @param G contact network
#' @param P transmission network
#' @param Ia vector of times of infection for each individual
#' @param Il vector of times of transition to long-term infection for each individual
...
#' @param gamma_a length of infection for acute phase
#' @param gamma_l length of infection for long-term phase
#' @return new_Ia, updated acute infection times
#' @export
...
Update_Ia <- function(G,P,Ia,Il,R,beta_a,beta_l,gamma_a,gamma_l) {
  ...
  new_Ia = array(Inf,dim=length(igraph::V(G)))

  for (j in infected) {

    parent_j = unlist(igraph::neighborhood(P, 1, nodes=j-1, mode="in"))[-1] + 1
    if(length(parent_j) >0){
      parent_Ia = Ia[parent_j]
      parent_R = R[parent_j]

      contact_neighbors = unlist(igraph::neighborhood(G, 1, nodes=j-1, mode="all")) + 1
...
#' @title Update_Il
#' @description updates long-term infection times
#' @param G contact network
#' @param P transmission network
#' @param Ia vector of times of infection for each individual
#' @param Il vector of times of transition to long-term infection for each individual
...
#' @param gamma_a length of infection for acute phase
#' @param gamma_l length of infection for long-term phase
#' @return new_Il, updated long-term infection times
#' @export
...
Update_Il <- function(G,P,Ia,Il,R,beta_a,beta_l,gamma_a,gamma_l) {
  ...
  new_Il = array(Inf,dim=length(igraph::V(G)))
...
#' @title Update_I
#' @description updates infection times (acute and long-term)
#' @param G contact network
#' @param P transmission network
#' @param Ia vector of times of infection for each individual
#' @param Il vector of times of transition to long-term infection for each individual
...
#' @param gamma_a length of infection for acute phase
#' @param gamma_l length of infection for long-term phase
#' @return list(Il = new_Il, Ia = new_Ia), updated infection times (acute and long-term)
#' @export
...
Update_I <- function(G,P,Ia,Il,R,beta_a,beta_l,gamma_a,gamma_l) {
  ...
  new_Ia <- new_Il <- array(Inf,dim=network::network.size(G))
  ...
  for (j in infected) {
    ...
    contact_neighbors = network::get.neighborhood(G, j, type = "combined")
    inf_Ia = Ia[j]
    inf_Il = Il[j]
    inf_R = R[j]
    ...
    parent_j = network::get.neighborhood(P, j, type = "in")
...
update_r <- function(r, g, P, Ia, Il, R_times, edge1, beta_a, beta_l) {
  ...
  add_edge = 1
  if (network::is.adjacent(g, edge1[1], edge1[2])) { 		#Remove edge
    add_edge = 0
  }
  ...
  if ((network::is.adjacent(P, edge1[1], edge1[2])) || (network::is.adjacent(P, edge1[2], edge1[1])) ) {
    return(0)
  } else {
