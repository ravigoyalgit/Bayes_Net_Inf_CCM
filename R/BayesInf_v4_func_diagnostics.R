#' @title genetic_seq_diagnostic
#' @description gives statistics on genetic sequence distances
#' @param P transmission network
#' @param T viral genetic network
#' @return list(mean(dist_linked_cases), mean(T_data[which(T_data > 0)]), var(dist_linked_cases), var(T_data[which(T_data > 0)])), mean and variance of genetic sequence distances
#' @export

genetic_seq_diagnostic <- function(P,T) {

  dist_linked_cases = c()
  for (i in c(1:igraph::ecount(P))) {
...
#' @return print(cor(Ia, Initial_Data[[3]])), print(cor(Il, Initial_Data[[4]])), correlations of infection data (acute and long-term) with those simulated
#' @export
...
accuracy_G_P_diagnostic <- function(P_a, G_a, Initial_Data) {
  ...
  adj_m = igraph::get.adjacency(P)
  for (i in c(51:length(P_a))) {
    adj_m  = adj_m + igraph::get.adjacency(P_a[[i]])
  }
  adj_m = adj_m / length(P_a)
  adj_P = igraph::get.adjacency(P_a[[1]])
  ...
  adj_Gm = igraph::get.adjacency(G_a[[2]])
  for (i in c(3:length(G_a))) {
    adj_Gm  = adj_Gm + igraph::get.adjacency(G_a[[i]])
  }
  adj_Gm = adj_Gm / length(G_a)
  adj_G = igraph::get.adjacency(G_a[[1]])
...
network_properties_diagnostic <- function(G_a, burnin, view_graphs = FALSE) {
  ...
  burnin = burnin + 1
  population = length(network::network.size(G_a[[1]]))

  ecount_truth = network::network.edgecount(G_a[[1]])
  assort_truth =  summary(G_a[[1]]~degcor)
  degree_dist_truth = tabulate(igraph::degree(G_a[[1]])/2+1)

  ecount_a = c()
  for (i in c(burnin:length(G_a))) {
    ecount_a = c(ecount_a, network::network.edgecount(G_a[[i]]))
  }
  ...
  if (view_graphs == TRUE) {
    ...
    plot_data = ecount_a/choose(network::network.size(G_a[[1]]),2)
    plot(stats::density(plot_data), main = "Density plot Network Property Edge Density", xlab = "Edge Density")
    abline(v=(ecount_truth/choose(network::network.size(G_a[[1]]),2)), col = "red")
    readline(prompt = "Pause. Press <Enter> to continue...")
  }

  max_deg = 0
  for (i in c(burnin:length(G_a))) {
    if (max_deg < max(igraph::degree(G_a[[i]])/2)) {
      max_deg = max(igraph::degree(G_a[[i]])/2)
    }
  }
  max_deg = max(max_deg, max(igraph::degree(G_a[[1]])/2))
  ...
  for (i in c(burnin:length(G_a))) {
    ...
    degree_dist_mat[index_i,c(1:length(tabulate(igraph::degree(G_a[[i]])/2+1)))] = tabulate(igraph::degree(G_a[[i]])/2+1)
  }
