dens_biased_sim <- function(init_seed, num_samples) {
  outfile = paste("dens_sampPropDeg_n", num_samples, "_", init_seed, ".RData", sep = "")
  ...
  Prob_Distr_Params[[1]][[2]] = matrix(c(2,0,0,0,2,0,0,0,2), nrow = 3, ncol = 3) #Variance
  ...
  degTemp <- degree(G)/ 2
  sample_deg <- sample(degTemp, num_samples, prob = (degTemp+1), replace = FALSE)
  ...
  save(ecount_G, ecount_P, DD_dist, degdist, mixstat, file = outfile)
}
