DD_unbiased_sim <- function(init_seed, num_samples) {
  ...
  degTemp <- degree(G)/ 2
  sample_deg <- sample(degTemp, num_samples, prob = 1/(degTemp+1))
  samp_dist <- c(sum(sample_deg == 0), tabulate(sample_deg, nbins = max(degree(G)/2)))
