dens_unbiased_sim <- function(init_seed, num_samples) {
  outfile = paste("dens_unbiased_n", num_samples, "_", init_seed, ".RData", sep = "")
  ...
  Network_stats = 'DegreeDist' #keep degree dist so we can do sampling on degrees.
  strong_prior = TRUE
  ...
  P = Initial_Data[[2]]
  Ia = Initial_Data[[3]]
  Ia[is.na(Ia)] = 999999+1
  Il = Initial_Data[[4]]
  Il[is.na(Il)] = 999999+1
  R = Initial_Data[[5]]
  R[is.na(R)] = 999999+1
  T_dist = Initial_Data[[6]]
  degTemp <- degree(G)/ 2
  sample_deg <- sample(degTemp, num_samples, replace = FALSE)
  target_dens = (mean(sample_deg))/(population-1)
  Prob_Distr_Params = list(list(target_dens, target_dens*(1-target_dens)/num_samples))
  print(paste("Target:", target_dens))
  print(paste("Obs:", network.density(G)))
  Prob_Distr = "Normal"
  ...
  G_start2 = CCMnet_constr(Network_stats="DegreeDist",
                           Prob_Distr="DirMult",
