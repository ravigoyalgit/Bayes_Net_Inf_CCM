mix_biasReport_sim <- function(init_seed, num_samples, mixpct = 1/3, biaspct = 1) {
  ...
  unbPct <- apply(edge_type_counts, 2, mean)[2]/sum(apply(edge_type_counts, 2, mean))
  newPct <- unbPct*biasPct
  svySampMean = sum(apply(edge_type_counts, 2, mean))*c((1-newPct)/2, newPct, (1-newPct)/2)
