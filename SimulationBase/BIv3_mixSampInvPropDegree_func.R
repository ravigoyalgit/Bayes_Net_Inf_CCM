mix_invProp_sim <- function(init_seed, num_samples, mixpct = 1/3) {
  outfile = paste("mix_sampInvProp_n", num_samples, "_highVar_", init_seed, ".RData", sep = "")
  ...
  Prob_Distr_Params[[1]][[2]] = matrix(c(1.5,0,0,0,1.5,0,0,0,1.5), nrow = 3, ncol = 3) #Variance
  ...
  degTemp <- degree(G)/ 2
  node_id = sample(c(1:network.size(G)), num_samples, replace = FALSE, prob = 1/(degTemp+1))
